...
int main(int argc, char **argv) 
{
   ...
   if (argc < 6 || argc > 7) {
      cerr << "usage: " << argv[0] << " <file1: state> <file2: reactions> <trials> <time> <verbosity> [max_events]" << endl;
      exit(1);
   } 
   ...
   param.max_events = 0;
   if (argc == 7) {
      param.max_events = atoi(argv[6]);
   }

   // for each trial
   vector<unsigned> A(S.size());
   vector<double>   Q(S.size());
   ...
   for (unsigned i = 0; i < A.size(); i++) {
      ...
      double var  = Q[i]/(double)trials - mean*mean;
      if (var < 0) var = 0;
      cout << var;
      if (i < A.size() - 1) cout << ", ";
   }
